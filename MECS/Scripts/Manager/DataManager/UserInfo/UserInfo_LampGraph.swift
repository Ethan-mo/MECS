...
class CastDateFormatLampGraph: CastDateFormat {
...
class LampGraphInfo {
    ...
    var m_hum: Int = 0
    var m_voc: Int = 0
    var m_castTimeInfo: CastDateFormatLampGraph!
    var m_statusInfo: LampStatusInfo?
    ...
    init(id: Int, did: Int, time: String?, tem: Int, hum: Int, voc: Int, originTime: String? = nil, addIndex: Int? = nil) {
        ...
        if (time != nil) {
            self.m_castTimeInfo = CastDateFormatLampGraph(time: time!)
        } else {
            self.m_castTimeInfo = CastDateFormatLampGraph(originTime: originTime!, addIndex: addIndex!)
        }
        self.m_time = self.m_castTimeInfo.m_time

        self.m_xAxisValue = setXAxisValue()
        
        if let _status = DataManager.instance.m_userInfo.deviceStatus.m_lampStatus.getInfoByDeviceId(did: m_did) {
            self.m_statusInfo = LampStatusInfo(did: m_did, name: "", power: 0, bright: 0, color: 0, attached: 1, temp: m_tem, hum: m_hum, voc: m_voc, ap: "", apse: "", tempmax: _status.m_tempmax, tempmin: _status.m_tempmin, hummax: _status.m_hummax, hummin: _status.m_hummin, offt: "", onnt: "", con: 0, offptime: "", onptime: "")
        }
    }
...
class UserInfo_LampGraph {
    var m_lst = [LampGraphInfo]()
    ...
    func isExist(item: LampGraphInfo) -> Bool {
        for _item in m_lst {
    ...
    func addItem(arr: [LampGraphInfo]?) {
        if (arr == nil) {
            return
        }
    ...
    func deleteItemByDid(did: Int) {
        var _delList = Array<LampGraphInfo>()
        for item in m_lst {
            if (item.m_did == did) {
        ...
        DataManager.instance.m_coreDataInfo.storeLampGraph.deleteItemsByDid(did: did)
    }
    ...
    func deleteItemById(id: Int) {
        ...
        DataManager.instance.m_coreDataInfo.storeLampGraph.deleteItemsById(id: id)
    }
}
