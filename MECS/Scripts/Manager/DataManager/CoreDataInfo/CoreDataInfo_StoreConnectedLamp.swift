...
// 연결된 장비 정보
class CoreDataInfo_StoreConnectedLamp {
    
    let m_entityName = "StoreConnectedLamp"
    
    func addItemToEntity(entity: NSEntityDescription, item: StoreConnectedLampInfo, isBackground: Bool = false) {
        let _coreData = DataManager.instance.m_coreDataInfo
        let _addItem = StoreConnectedLamp(entity: entity, insertInto: isBackground == true ? _coreData.backgroundContext : _coreData.context)
    ...
    func deleteItemsLoginAid(loginAid: Int) {
        ...
        do
        {
            ...
            Debug.print("[ERROR] StoreConnectedLamp Error with request: \(error)", event: .error)
        }
    }
    ...
    func updateItem(info: StoreConnectedLampInfo) {
        let _coreData = DataManager.instance.m_coreDataInfo
        let fetchRequest = NSFetchRequest<NSFetchRequestResult>(entityName: m_entityName)
        do{
            let objects = try _coreData.context.fetch(fetchRequest)
            if objects.count > 0 {
                for item in objects {
                    let match = item as! NSManagedObject
                    let _type = Int(_coreData.getValueString(obj: match, key: "type")) ?? DEVICE_TYPE.Lamp.rawValue
                    let _did = _coreData.getValueInt(obj: match, key: "did")
                    let _login_aid = _coreData.getValueInt(obj: match, key: "login_aid")
                    if (_did == info.did && _login_aid == info.login_aid) {
    ...
    func addItem(item: StoreConnectedLampInfo, isBackground: Bool = false) {
        let _coreData = DataManager.instance.m_coreDataInfo
    ...
    func addItems(list: Array<StoreConnectedLampInfo>, isBackground: Bool = false) {
        let _coreData = DataManager.instance.m_coreDataInfo
        let _entity = _coreData.getEntity(name: m_entityName)
        for item in list {
            addItemToEntity(entity: _entity!, item: item, isBackground: isBackground)
    ...
    func load() -> Array<StoreConnectedLampInfo>? {
        let _coreData = DataManager.instance.m_coreDataInfo
        var _arrData = [StoreConnectedLampInfo]()
        ...
        do{
            ...
            if objects.count > 0 {
                for item in objects {
                    ...
                    let _info = StoreConnectedLampInfo(adv: _adv,
                                                         uuid: _uuid,
                                                         type: _type,
                                                         did: _did,
                                                         srl: _srl,
