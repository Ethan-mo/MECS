...
class CoreDataInfo_StoreLampGraph {
    
    let m_entityName = "StoreLampGraph"
    var m_lastId: Int = 0
    var lastId: Int {
        get {
            m_lastId += 1
    ...
    func addItemToEntity(entity: NSEntityDescription, item: LampGraphInfo, isBackground: Bool = false) -> Int {
        let _coreData = DataManager.instance.m_coreDataInfo
        let _addItem = StoreLampGraph(entity: entity, insertInto: isBackground == true ? _coreData.backgroundContext : _coreData.context)
        let _lastId = lastId
    ...
    func deleteItemsByDid(did: Int) {
        ...
        do
        {
            ...
            Debug.print("[ERROR] StoreLampGraph Error with request: \(error)", event: .error)
        }
    }
    ...
    func updateItem(info: LampGraphInfo) {
        let _coreData = DataManager.instance.m_coreDataInfo
        let fetchRequest = NSFetchRequest<NSFetchRequestResult>(entityName: m_entityName)
        do {
            let objects = try _coreData.context.fetch(fetchRequest)
    ...
    func load() -> Array<LampGraphInfo>? {
        let _coreData = DataManager.instance.m_coreDataInfo
        var _arrData = [LampGraphInfo]()
        ...
        do{
            ...
            if objects.count > 0 {
                for item in objects {
                    ...
                    let _info = LampGraphInfo(id: _id ?? -1,
                                               did: _did ?? 0,
        ...
        do
        {
            ...
            for item in objects
            {
                ...
                if (NSDate(timeIntervalSinceNow: TimeInterval(-86400 * 8)) as Date > CastDateFormatDeviceNoti(time: _time).m_timeCast) {
                    Debug.print("StoreLampGraph DeleteOldData: \(String(describing: CastDateFormatDeviceNoti(time: _time).m_timeCast))", event: .dev)
                    _coreData.context.delete(match)
                }
            }
            _coreData.saveData()
